...
function queryDatabase(usrname, hashpw) {
  var errorsGoHere = document.getElementById("errors");
  // Read all rows from table
  const request = new Request(
    "INSERT INTO Authenticators (Username, Password) VALUES (" + usrname + ", " + hashpw + ");",
    (err, rowCount) => {
      if (err) {
        errorsGoHere.innerHTML = err.message;
      } else {
        errorsGoHere.innerHTML = "Success!";
      }
    }
  );
...
function makeAccount() {
  ...
  if (password1.value === password2.value) {
    var hashpw = sha512(password1.value);
    queryDatabase(username.value, hashpw);
    output.innerHTML = "DB successfully appended.";
  }
  else {
